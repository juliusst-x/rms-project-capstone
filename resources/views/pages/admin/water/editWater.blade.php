...
<main class="h-full pb-16 overflow-y-auto">
	<div class="container px-6 mx-auto grid">
		...
		<form action="{{ route('water.edit', $water->identifiers) }}" method="POST" enctype="multipart/form-data">
			@csrf
			<div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
				...
				<label class="block mt-4 text-sm">
					...
					<select
						...
						<option value="{{ $area->id }}" @if ($area->id==$water->area_id){ selected } @endif>
							{{ $area->area_name }}
						</option>
						@endforeach
					</select>
				...
				<label class="block mt-4 text-sm">
					...
					<input
						...
						type="number" placeholder="Threshold" value="{{$water->threshold}}" name="Threshold"
						id='Threshold'>
				</label>
